...
| Improve exact-source parity with jCodeMunch | Ready to merge; owned by this task | `codex/improve-exact-source-parity` | `.worktrees/improve-exact-source-parity` | `origin/main` at `cc23d84` | Required CI passed on implementation head `cea92bb`; corrected benchmark is 3/3 per server | Merge PR #150, verify main, then archive the OpenSpec change |
