...
| JSON repository configuration | `src/config.ts` reads `astrograph.config.json` directly. It is documented and covered by engine, CLI, event, and watch fixtures. | Retain as the active public configuration contract; production commands do not execute repository TypeScript. |
