...
> Migrated from [the legacy active tracker](../../../specs-legacy/implementation/active/11_wasm-parser-runtime-migration.md). The checkbox state below is authoritative going forward.
...
- [x] Assert a clean install of the packed tarball resolves the runtime and
  every selected grammar asset, and no native Tree-sitter dependency. npm
  tarballs do not contain ordinary dependency assets themselves.
- [x] Run the packed global-install smoke locally under ASDF Node 20.19 and
  24.13.
- [ ] Dispatch the Linux Node 20 and 24 manual package gates.
