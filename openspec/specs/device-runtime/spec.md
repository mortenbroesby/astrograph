...
client or project isolation reliability. Client guidance SHALL account for the
daemon's per-repository safety queue.

The managed runtime SHALL reuse its selected Node executable across
client-driven upgrades and SHALL run package installation and native rebuild
lifecycle scripts with that Node's bin directory first on `PATH`.
...
- **WHEN** an agent needs multiple Astrograph results from the same canonical
  repository root
- **THEN** client guidance directs it to issue those operations sequentially
- **AND** ordinary requests do not expire solely because the client created a
  same-repository parallel burst
...
- **WHEN** failures remain attributable to shared daemon ownership after the
  stable runtime, bounded recovery, and documented request sequencing are in
  place
...
- **WHEN** a device already has a healthy managed runtime and another client
  upgrades it from a different ambient Node version
- **THEN** Astrograph retains the active descriptor's Node executable
- **AND** npm install and native rebuild scripts execute with that Node's bin
  directory first on `PATH`
- **AND** runtime verification loads native dependencies under the same ABI
  before client configuration is changed

#### Scenario: A client rewrites Codex TOML comments

- **WHEN** the Astrograph block still selects an immutable entrypoint inside the
  managed runtime root but its management comments are missing or displaced
