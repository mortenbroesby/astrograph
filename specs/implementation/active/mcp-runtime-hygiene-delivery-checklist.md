...
- [ ] **Step 6: Verify release scope and commit checkpoint.**
  ...
  Focused tests and type checks pass. The current worktree has unrelated dirty
  files with README conflict markers, so use a scoped diff check and stage only
  this story before the final version/release decision and commit. This is a
  compatible runtime reliability fix and is expected to require a patch release
  when staged. Do not alter unrelated dirty files.
