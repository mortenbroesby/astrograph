...
- `package.json` currently declares `node >=22.12.0`; README currently says
  Node 24 is supported. The existing required CI uses Node 22.
...
- [ ] Run clean `pnpm install --frozen-lockfile`, build, type lint, focused
  runtime tests, `pnpm pack`, and `pnpm test:package-bin` using pinned Node 20,
  22, and 24 binaries.
