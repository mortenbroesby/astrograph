...
function agentsPolicyBlockForAgentsMd(): string {
  return [
    ...
    "### Working agreements",
    "",
    "- Start with `get_project_status`. If the index is missing or unavailable, run `index_folder` once, wait for it to finish, then retry the Astrograph request. If it is deep-ready but degraded, use its safe operations and recommended action instead of repeating `index_folder`.",
    "- Call Astrograph tools for one repository sequentially; calls for different repositories may run concurrently.",
    ...
    "- Keep retrieval bounded: start `search_symbols` with an exact or file-scoped query and limit 5, `get_task_context` with a 1,200-token budget, and `get_symbol_source` with one or two ids; increase only after refining.",
