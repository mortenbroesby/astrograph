...
`,
  ...
  it("anchors indexing and diagnostics to a spaced enclosing git worktree root", async () => {
    ...
    expect(health.storageVersion).toBe(3);
  ...
  it("resets version-2 storage before serving version-3 diagnostics", async () => {
    const repoRoot = await createFixtureRepo({
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ version: 2, updatedAt: "2026-07-18T00:00:00.000Z" }),
    );
